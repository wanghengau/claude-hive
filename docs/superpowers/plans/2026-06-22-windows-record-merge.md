...
**Architecture:** record-proxy 核心逻辑 TS 重写为 windows server 的同进程模块（`record-proxy.ts` + `record-store.ts`）；windows server 单端口（4000）同时承载 web UI、WebSocket、claude API 代理（录制）、录制查看 API；`claude-record` profile 通过 `ANTHROPIC_CUSTOM_HEADERS` 注入 `X-Window-Id` 让代理识别来源窗口；普通 `claude` 不改 BASE_URL 故不经代理（"不加载"）。
...
```
启动: cd windows-record && npm run dev  (浏览器开 http://localhost:4000，新建一个窗口 wmt-xxx)
...
浏览器开 http://localhost:4000 → 新建窗口 A（wmt-xxx）
```
